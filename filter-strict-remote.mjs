...
// Companies already engaged (any status: Applied, Evaluated, Discarded)
...
function getLocationType(j, type) {
  // Try to infer location type (Remote / Hybrid / On-site)
  if (type === 'ashby') return j.workplaceType || j.locationType || '';
  if (type === 'greenhouse') return ''; // GH doesn't always expose this
  return '';
}
...
// AI/ML/agent/forward-deployed/backend engineering
const ROLE_RE = /\b(software engineer|backend engineer|fullstack engineer|full[- ]stack engineer|ai engineer|ml engineer|machine learning engineer|applied ai|llm engineer|agent.*engineer|forward deployed engineer|solutions architect|solutions engineer|product engineer)\b/i;
const HARD_EXCLUDE_RE = /\b(research scientist|hardware|silicon|firmware|sales engineer|account|recruiter|talent|gtm representative|customer success|legal|policy|finance|founding)\b/i;

// STRICT location: must be unambiguously Remote-US or in the candidate's local substrings (config/profile.yml).
// Build the local matcher from config/profile.yml § location_policy.acceptable_local_substrings,
// falling back to location.city / location.state when that section isn't set yet.
...
const NON_LOCAL_CITIES = /\b(san francisco|sf bay|bay area|new york|nyc|brooklyn|manhattan|seattle|bellevue|redmond|boston|cambridge|los angeles|la,? ca|chicago|denver|miami|atlanta|austin,?\s*tx|plano|dallas|frisco|irving|dfw)\b/i;
...
function isAcceptableLocation(loc, locType) {
  ...
  // Hard exclude any location with non-US region
  if (NON_US_REGIONS.test(lc)) return { ok: false, reason: 'non-US region' };

  // Local-friendly per config/profile.yml § location_policy.acceptable_local_substrings
  if (LOCAL_RE && LOCAL_RE.test(lc)) return { ok: true, kind: LOCAL_LABEL };

  // Reject if anchored to a non-local US city, even if "remote" is mentioned
  if (NON_LOCAL_CITIES.test(lc) && (!LOCAL_RE || !LOCAL_RE.test(lc))) return { ok: false, reason: `anchored to non-local city` };

  // Reject Hybrid / On-site location types unless local (handled above)
  if (lt === 'on-site' || lt === 'hybrid' || lt === 'in-office') return { ok: false, reason: `location_type=${lt}` };

  // Accept if explicitly Remote US
  ...
  if (/^us\s*\|\s*remote|^usa\s*\|\s*remote/i.test(lc)) return { ok: true, kind: 'REMOTE-US' };
  if (/\bus\s*remote\b/i.test(lc)) return { ok: true, kind: 'REMOTE-US' };
  if (/\bremote\s+(in\s+)?(the\s+)?(us|united states)/i.test(lc)) return { ok: true, kind: 'REMOTE-US' };
  if (/\b(remote|telecommute)\b/i.test(lc) && /\b(us|united states|america|texas|north america)\b/i.test(lc)) return { ok: true, kind: 'REMOTE-US' };

  // Reject everything else
  return { ok: false, reason: `unclear location: "${loc}"` };
}
...
console.log(`Found ${matches.length} STRICTLY remote/local Senior-IC AI engineering roles (excluding companies already in tracker):\n`);
let last = '';
