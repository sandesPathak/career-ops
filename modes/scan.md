...
>
> **Nota (v1.6+) — multi-source scrapers:** Tres scanners adicionales se ejecutan al lado de `scan.mjs`, todos zero-token:
>
> - **`scan-aggregators.mjs`** (`npm run scan:aggregators`) — Indeed/Google/ZipRecruiter vía la librería Python JobSpy. Lee `aggregator_queries` de `portals.yml`. Requiere venv Python (`npm run setup:python`). En 2026-Q1 sólo Indeed retorna resultados de forma fiable; Google y Zip se mantienen configurados pero retornan 0 filas (Cloudflare WAF / consent redirect a nivel de red, no es un bug del parser).
> - **`scan-himalayas.mjs`** (`npm run scan:himalayas`) — himalayas.app vía API JSON pública. Lee `himalayas_queries` de `portals.yml`. Pure Node, sin proxies, sin Python. Filtra por `locationRestrictions` que contengan "United States" cuando `himalayas_us_strict: true` (default).
> - **`scan-curated.mjs`** (`npm run scan:curated`) — listas comunitarias `SimplifyJobs/New-Grad-Positions` y `SimplifyJobs/Summer2026-Internships` en GitHub. Auto-descarte de roles que requieren ciudadanía 🇺🇸 cuando `config/profile.yml § work_authorization.status` no es `"US Citizen"`. Yield bajo para perfiles senior — la cobertura es para early-career.
> - **Comando combinado:** `npm run scan:all` ejecuta `scan` + `scan:aggregators` + `scan:curated` + `discover` en serie. Apto para `/loop 1h /career-ops scan`.
>
> Todos los scanners escriben a la misma `data/pipeline.md` y `data/scan-history.tsv` (con columna `last_seen` para rastrear refrescos). El filtrado se centraliza en `scan-core.mjs` con `title_filter`, `location_filter`, y `company_filter` (auto-descarte de contratistas federales/defensa).
