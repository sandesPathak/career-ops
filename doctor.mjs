...
// Optional check — only fails if portals.yml has aggregator_queries configured.
// Otherwise the venv is unused and skipping it is fine.
async function checkPythonVenv() {
  const portalsPath = join(projectRoot, 'portals.yml');
  if (!existsSync(portalsPath)) {
    return { pass: true, label: 'Python venv: not needed (no portals.yml)' };
  }
  let hasAggregators = false;
  try {
    const { default: yaml } = await import('js-yaml');
    const fs = await import('node:fs/promises');
    const cfg = yaml.load(await fs.readFile(portalsPath, 'utf-8'));
    hasAggregators = (cfg?.aggregator_queries || []).length > 0;
  } catch {
    return { pass: true, label: 'Python venv: skipped (yaml parse failed)' };
  }
  if (!hasAggregators) {
    return {
      pass: true,
      label: 'Python venv: not needed (no aggregator_queries)',
    };
  }
  if (existsSync(join(projectRoot, '.venv', 'bin', 'python3'))) {
    return { pass: true, label: 'Python venv ready (scan-aggregators.mjs)' };
  }
  return {
    pass: false,
    label: 'Python venv missing — scan-aggregators.mjs will fail',
    fix: 'Run: npm run setup:python',
  };
}
