...
// ── Keyword-match hard gate ─────────────────────────────────────────
//
// Per-application tailored CVs (cv-{slug}-{date}.html) MUST run
// match-keywords.mjs first. The gate:
//   1. Detects per-application HTML by filename pattern
//   2. Looks for sibling .keywords.json
//   3. Refuses to generate PDF if missing or if overlap < threshold
//   4. Bypass with --skip-keyword-gate (intended only for the canonical base CV)
function checkKeywordGate(inputPath, args) {
  if (args.includes('--skip-keyword-gate')) {
    console.log('⚠️  Keyword gate bypassed (--skip-keyword-gate)');
    return;
  }
  const fname = basename(inputPath);
  // Per-application: cv-{slug}-{YYYY-MM-DD}.html, but NOT cv-base-{date}.html (canonical 1-pager).
  const isPerApplication = /^cv-.+-\d{4}-\d{2}-\d{2}\.html$/.test(fname)
    && !fname.startsWith('cv-base-');
  if (!isPerApplication) return;

  const keywordsPath = inputPath.replace(/\.html$/i, '.keywords.json');
  if (!existsSync(keywordsPath)) {
    console.error('❌ KEYWORD GATE FAILED');
    console.error(`   ${keywordsPath} not found.`);
    console.error(`   Run first:`);
    console.error(`     node match-keywords.mjs <JD-URL> ${inputPath}`);
    process.exit(2);
  }
  const report = JSON.parse(readFileSync(keywordsPath, 'utf-8'));
  if (!report.pass) {
    console.error('❌ KEYWORD GATE FAILED');
    console.error(`   Overlap ${report.overlap_percent}% < threshold ${(report.threshold * 100)}%`);
    console.error(`   Add more JD-aligned terms to Skills section, then re-run match-keywords.mjs`);
    process.exit(2);
  }
  console.log(`✓ Keyword gate passed: ${report.overlap_percent}% overlap (${report.counts.matched}/${report.counts.jd_terms_total} JD terms matched)`);
}
...
async function generatePDF() {
  ...
  // ── HARD GATE: keyword-match must pass for per-application CVs ──
  checkKeywordGate(inputPath, args);
